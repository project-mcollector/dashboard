...
function renderProjects() {
  ...
  filtered.forEach(proj => {
    ...
    toggleBtn.title = isVisible ? 'Скрыть ключ' : 'Показать ключ';
    toggleBtn.innerHTML = isVisible
      ? '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>'
      : '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
    toggleBtn.addEventListener('click', () => toggleKey(proj.id));
    ...
    manageBtn.title = 'Управление ключом';
    manageBtn.innerHTML = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.87"/></svg>';
    manageBtn.addEventListener('click', () => openApiKeyModal(proj.id));
...
async function deleteAccount() {
  ...
  try {
    await authFetch(`${API_URL}/api/users/me`, { method: 'DELETE' });
    clearAuth();
...
async function confirmRename() {
  ...
  try {
    const res = await authFetch(`${API_URL}/api/projects/${renameTarget}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
async function doRegenerate() {
  ...
  try {
    const res = await authFetch(`${API_URL}/api/projects/${apiKeyTarget.id}/api-key/regenerate`, {
      method: 'POST'
    });
    const updated = await res.json();
    currentProjects = currentProjects.map(p => p.id === updated.id ? updated : p);
